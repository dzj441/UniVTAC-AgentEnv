...
if [[ "${HAS_TASK}" == true || "${HAS_PROFILE}" == true ]]; then
  if [[ "${HAS_TASK}" != true || "${HAS_PROFILE}" != true || "${HAS_LEVEL}" == true ]]; then
    echo "generic usage: $0 --task TASK --profile {1..6} [--pre-move] [--provide-bbox] [--provide-mask] [--key-initial-relative-yaw-rad RAD] [--post-action-settle-steps STEPS] [--[no-]sim-step-recorder] [--sim-step-recorder-fps FPS] [--sim-step-recorder-post-action-steps STEPS] [--device cuda:0] [--run-dir PATH]" >&2
    exit 2
  fi
